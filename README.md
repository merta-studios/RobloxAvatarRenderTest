...
- **„Kein Fortschritt … in Phase „assets“ ohne „Zuletzt geladen:“** – Zeigt die EMPTY-Label-Variante des Hängers: Die Downloads waren längst fertig, aber ein interner Promise-Pfad der Bibliothek hat nie aufgelöst (`roavatar-renderer` 1.6.2 verwendet `.then(resolve)`-Ketten ohne Rejection-Handler). Dagegen liegen jetzt vier Schichten: 1. `guardGetRBX` (Rejection/Deadline um `API.Asset.GetRBX`, z. B. wenn `rbx.fromBuffer()` auf korrupten RBXM-Formaten wirft), 2. Runtime-Patches der exportierten Prototype-Klassen `AnimatorWrapper.loadAnimation`/`loadAvatarAnimation`/`playAnimation` und `HumanoidDescriptionWrapper.applyDescription` (u. a. der „Animation was already loaded“-Wettlauf, wenn zwei Animation-Sets dieselbe Sub-Animation teilen – Throws werden zu „Asset übersprungen“ statt zum stillen Hänger; kein Eingriff in `node_modules`), 3. eine Stall-erkennende Deadline um `prepareForThumbnail` (200 s ohne Fortschritt bzw. 400 s gesamt – unter dem 240-s-Watchdog, damit die konkrete Meldung gewinnt; langsame Renders mit Fortschritt laufen weiter), 4. der Watchdog als letzte Auffanglinie. Schlägt ein Render trotzdem fehl, zeigt die Discord-Antwort unter **Diagnose** jetzt Build-ID (erkennt veraltete Deploys), fehlgeschlagene Requests und Console-Fehler.
