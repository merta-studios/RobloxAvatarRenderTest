...
- **Deterministische Thumbnail-Vorbereitung:** Statt unbegrenzt auf die `onSuccess`/`onRenderSuccess`-Events der Bibliothek zu warten, läuft die Vorbereitung als eigene Zustandsmaschine mit Stufen-Deadlines (`waiting-outfit` → `posing` → `waiting-render-compile`). Die Render-Kompilierung wird am echten `RenderScene`-Zustand gepollt. Ein einzelner dauerhaft hängender oder fehlgeschlagener Render-Descriptor (defektes Accessoire/Mesh/Textur) wird gezielt entfernt und der Rest-Avatar trotzdem gerendert (Degraded-Render mit Hinweis) – der alte 200-s-Stall `assets|` kann so nicht mehr auftreten.
...
- **Phasen-Logs:** Jede Render-Phase wird mit Laufzeit in den Server-Logs protokolliert (`[render] userId=… Phase assets – … (+42 s)`). Bei einem Timeout nennt die Discord-Antwort die letzte Phase. Die internen Vorbereitungs-Teilschritte (`prepareStage`) samt Descriptor-Zählern und pending-Instanzen stehen in `window.__renderState`, im `/render-debug`-JSON und in den Fehler-Logs.
